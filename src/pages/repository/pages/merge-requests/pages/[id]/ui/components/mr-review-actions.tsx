  ...
  return (
    <div className="flex items-center gap-2 text-[13px]">
      <ReviewerAvatar
        ...
        avatarUrl={reviewer.user.avatar_url ?? null}
      />
...
export const MrReviewActions = ({
  ...
  if (mergeRequest.state !== "opened" || isDraft(mergeRequest)) {
    return null;
  }
  ...
  const approvalsSummary =
    approvals.approvals_required !== null
      ? `${approvals.approved_by.length}/${approvals.approvals_required} апрувов`
      : approvals.approved_by.length > 0
        ? `${approvals.approved_by.length} апрув(ов)`
        : null;
    ...
    <section className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-white px-5 py-4 dark:border-slate-800 dark:bg-gray-900">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {approvals.current_user_requested_changes ? (
            <button
              type="button"
            ...
            <button
              type="button"
              className={`${actionButtonClass} border border-red-300 bg-white text-red-700 hover:bg-red-50 dark:border-red-800 dark:bg-canvas-default dark:text-red-300 dark:hover:bg-red-950`}
              disabled={isBusy}
          ...
          {approvals.approvals_available &&
            (approvals.current_user_approved ? (
              <button
                type="button"
              ...
              <button
                type="button"
                className={`${actionButtonClass} border-none bg-success-emphasis text-white hover:bg-success-emphasis-hover`}
                disabled={isBusy}
                onClick={onApprove}
              >
      ...
      {approvals.approved_by.length > 0 && (
        <div className="flex flex-col gap-2 border-t border-slate-100 pt-3 dark:border-slate-800">
          ...
          <div className="flex flex-wrap gap-3">
            {approvals.approved_by.map((entry) => (
